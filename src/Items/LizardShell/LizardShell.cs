...
class LizardShell : PlayerCarryableItem, IDrawable
{
    ...
    public override void Update(bool eu)
    {
        ...
        bodyChunks[0].rad = abstractLizardShell.headBodyChunkRadius;
        base.collisionLayer = 2;
        ...
        if (grabbedBy.Count > 0)
        {
            ...
            if (grabber is Player && (grabber as Player).Sneak > 0.01f)
            {
                ...
                rotation = Vector2.Lerp(rotation, Custom.DirVec(grabber.mainBodyChunk.pos + scugAimDir * 2, base.firstChunk.pos), 0.3f);
            }
            else
            {
                // TODO: make this work with rotation in front and behind the player.
                rotation = Custom.PerpendicularVector(Custom.DirVec(base.firstChunk.pos, grabber.mainBodyChunk.pos));
    ...
    public void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
    {
        ...
        // Eyes are just a slightly darker version of shell to show the head is gone. X_X
        sLeaser.sprites[TotalSprites - 1].color = Color.Lerp(abstractLizardShell.shellColor, new Color(0f, 0f, 0f), 0.5f);
    ...
    public void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        float finalJawRotation = Mathf.Lerp(lastJawRotation, headRotation + desiredJawRotation, 0.25f);

        // Make sure the jaw doesn't rotate too far INSIDE of the head.
        if (finalJawRotation > headRotation + 10)
            finalJawRotation = headRotation + 10;
        ...
        lastJawRotation = finalJawRotation;
